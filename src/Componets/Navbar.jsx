...
const navbarData = [
  ...
  {
    label:  <IoSearchCircleSharp className='text-2xl ' />,
...
function Navbar() {
 ...
 <a href="https://flowbite.com/" className="flex items-center">
        ...
        <div className="flex md:order-2 space-x-3">
          <button
            type="button"
            className="text-white bg-yellow-600 hover:bg-yellow-800 focus:ring-4 focus:outline-none focus:ring-blue-300 font-medium rounded-full text-sm px-4 py-2"
          >
            Contact Us
          </button>
          <button
        ...
        >
          {!isSearchActive ? (
            <ul className="flex flex-col gap-2 font-medium md:p-0 mt-3 hover:text-yellow-800 rounded-lg bg-gray-50 md:flex-row md:mt-0 md:border-0 md:bg-black">
              {navbarData.map((menu, index) => (
                <li className="relative group " key={index}>
                  <p
                    className="flex items-center text-white  cursor-pointer m-0 px-2 "
                    onClick={() => {
                      if (menu.label.type === IoSearchCircleSharp) setIsSearchActive(true);
                    }}
                  >
                    {menu.label}
                    {!React.isValidElement(menu.label) && menu.dropdown.length > 0 && (
                      <svg
                        className="w-4 h-4 ml-1 transform hover:text-yellow-400 group-hover:rotate-180 transition-transform"
                        fill="none"
                        stroke="currentColor"
                        xmlns="http://www.w3.org/2000/svg"
                        viewBox="0 0 24 24"
                 ...
                 {menu.dropdown.map((item) => (
                   <li
                     className=" hover:text-yellow-800 dark:hover:bg-gray-700"
                     key={item.label}
                   >
